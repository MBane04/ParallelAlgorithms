...
void SetUpCudaDevices()
{
	...
	BlockSize.x = 2*prop.multiProcessorCount; //Make the block size twice the number of multiprocessors.
...
{
	__shared__ int bin[NUMBER_OF_BINS];
	int tid = threadIdx.x;
	...
	//find the bin that my value belongs in in shared memory
	if(id < NUMBER_OF_RANDOM_NUMBERS)
	{
		float binSize = MAX_RANDOM_NUMBER/(float)NUMBER_OF_BINS; //what is the size of each bin
		float myValue = randomNumbers[id]; //what is my value

		//find the bin that my value belongs in
		int myBin = min((int)(myValue/binSize), NUMBER_OF_BINS - 1); //my bin is my value divided by the size of each bin, min is used to make sure that the value is not bigger than the number of bins -1 bc we start at 0
		//NOTE: The line above works only if our bin starts at 0. If our bin starts at 1, we need to subtract 1 from myBin and so on.
		atomicAdd(&bin[myBin], 1); //add 1 to my bin in shared memory

	}
	__syncthreads();

   //add all shared vals to global
    if (tid < NUMBER_OF_BINS)
    {
        atomicAdd(&hist[tid], bin[tid]);
    }
